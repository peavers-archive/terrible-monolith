...
public class ThumbnailServiceImpl implements ThumbnailService {
    ...
    /**
     * Given a video file, divide its time length into the number of thumbnails to create, using FFMPEG jump to those
     * time stamps to grab the closest frame we find.
     */
    @Override
        ...
        for (int i = 1; i <= count; i++) {
            ...
            try {
                processService.execute(
                        CommandUtil.createThumbnail(String.valueOf(round(timestamp)), input.toFile().getAbsolutePath(),
    ...
    /**
     * Use FFMPEG to calculate the total duration of the video. This is used to work out where to create the
     * thumbnails.
     */
    private double calculateDuration(final Path input) {

        try {
            final String output = processService.execute(CommandUtil.calculateDuration(input.toFile().getAbsolutePath()));
    ...
    /**
     * Attempt to create the output directory. If this fails, we break all rules and hard abort the task as we don't
     * care anymore. Don't really do this in the real world.
     */
